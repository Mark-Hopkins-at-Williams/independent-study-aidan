...
def get_training_paths_for_lang_codes(config, target_lang_codes):
    lang_code_to_paths = {}
    target_set = set(target_lang_codes)  # Convert to set for faster lookup
    ...
    for corpus_name, corpus_data in config['corpora'].items():
        for lang_key, lang_info in corpus_data.items():
            ...
            # Only process if this lang_code is in our target subset
            if lang_code in target_set:
                train_files = lang_info['train']
                
                if lang_code not in lang_code_to_paths:
                    lang_code_to_paths[lang_code] = []
                    
                if isinstance(train_files, list):
                    lang_code_to_paths[lang_code].extend(train_files)
                else:
                    lang_code_to_paths[lang_code].append(train_files)
                    
    return lang_code_to_paths
...
def get_spanish_paths_for_lang_codes(config, target_langs):
    corpus_to_spanish_paths = {}
    target_set = set(target_langs)
    
    for corpus_name, corpus_data in config['corpora'].items():
        lang_keys = list(corpus_data.keys())
        # Assuming first language is target, second is Spanish
        if len(lang_keys) >= 2:
            first_lang_code = corpus_data[lang_keys[0]]['lang_code']
            second_lang_info = corpus_data[lang_keys[1]]
            
            # If first language is in our target set, get the Spanish path from second language
            if corpus_name in target_set:
                spanish_train_files = second_lang_info['train']
                
                if corpus_name not in corpus_to_spanish_paths:
                    corpus_to_spanish_paths[corpus_name] = []
                
                if isinstance(spanish_train_files, list):
                    corpus_to_spanish_paths[corpus_name].extend(spanish_train_files)
                else:
                    corpus_to_spanish_paths[corpus_name].append(spanish_train_files)
    
    return corpus_to_spanish_paths
...
def main():
    ...
    #updating the tokenizers sentence piece wise:
    add_new_tokens = params['add_new_tokens'] if 'add_new_tokens' in params else False
    if add_new_tokens:
        os.makedirs(f"{model_dir}/tokenizer_files")
        retrain_files = get_training_paths_for_lang_codes(config, langs_to_add)
